...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let lightboxAutoTimer = null;
    let preloadedImages = new Map();
    let cart = [];
    let cartCount = 0;
    // Load cart from localStorage on start
    try {
        const savedCart = localStorage.getItem('syrxty_cart');
        if (savedCart) {
    ...
    function openLightbox(images, startIndex) {
        ...
        if (lightboxAutoTimer) clearInterval(lightboxAutoTimer);
    ...
    function closeLightbox() {
        ...
        document.body.style.overflow = 'hidden';
        if (lightboxAutoTimer) {
            clearInterval(lightboxAutoTimer);
            lightboxAutoTimer = null;
        }
    }
    ...
    preloadEssentialImages().then(async () => {
        ...
        window.onNewImagesDownloading = function(count) {
            if (cacheInitialized) {
                cacheManager.showCacheNotification(`Downloading ${count} new images...`);
            }
        };

        window.onNewImagesReady = async function(count) {
            try {
                if (cacheInitialized) {
                    await refreshCountsAndSyncCache();
                    cacheManager.showCacheNotification(`✅ ${count} images ready!`, 'success');
                }
            } catch (e) {
                console.warn('Failed to refresh after new images:', e);
            }
        };
    });
    ...
    function updateCartUI() {
        ...
        try {
            localStorage.setItem('syrxty_cart', JSON.stringify(cart));
        } catch (e) {
